...
from crops import extract_crops_z, extract_crops_x, pad_frame, resize_images
sys.path.append('../')
sys.path.append(sys.path[0])

# re_h = tf.placeholder(tf.int32)
# re_w = tf.placeholder(tf.int32)
...
def build_tracking_graph(final_score_sz, design, env):
    ...
    # image = tf.image.resize_images(image,[re_h,re_w])
    image = 255.0 * tf.image.convert_image_dtype(image, tf.float32)
    # image = tf.image.resize_images(image,[re_w,re_h])
    ...
    return filename, image, templates_z, scores_up, scores
