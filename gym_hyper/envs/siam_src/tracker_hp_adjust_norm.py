...
def tracker(hp, run, design, frame_name_list, pos_x, pos_y, target_w, target_h, final_score_sz, filename, image, templates_z, scores, start_frame, mu_model, env, score_low):
    ...
    for i in range(1, num_frames):
        ...
        s0 = score_low_[1, :, :]/10
        t0 = time.time()
        u = mu_model.predict(np.reshape(s0,(1,1,score_low_.shape[1],score_low_.shape[2])))
        t1 = time.time()
        t_elapsed_u += t1-t0
        ...
        if u_stype == 'u5':
            ...
            hp = hp._replace(scale_step=u[0], scale_penalty=u[1], scale_lr=u[2], window_influence=u[3])
        elif u_stype == 'u4_slr':
            hp = hp._replace(scale_step=u[0], scale_penalty=u[1], window_influence=u[2], z_lr=u[3])
        elif u_stype == 'u4_win':
            hp = hp._replace(scale_step=u[0], scale_penalty=u[1], scale_lr=u[2], z_lr=u[3])
        elif u_stype == 'u4_ss':
            hp = hp._replace(scale_penalty=u[0], scale_lr=u[1], window_influence=u[2], z_lr=u[3])
        elif u_stype == 'u4_sp':
            hp = hp._replace(scale_step=u[0], scale_lr=u[1], window_influence=u[2], z_lr=u[3])
        scale_factors = hp.scale_step**np.linspace(-np.ceil(hp.scale_num/2), np.ceil(hp.scale_num/2), hp.scale_num)
    ...
    # print('average predict time:' + str(t_elapsed_u/(num_frames-1)))
