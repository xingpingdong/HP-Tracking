...
def tracker(opts,sess):
    global scores_, image_
    hp = opts.hp
    run = opts.run
    design = opts.design
    frame_name_list = opts.frame_name_list
    pos_x = opts.pos_x
    pos_y = opts.pos_y
    target_w = opts.target_w
    target_h = opts.target_h
    final_score_sz = opts.final_score_sz
    filename = opts.filename
    image = opts.image
    templates_z = opts.templates_z
    templates_z_ = opts.templates_z_
    scores = opts.scores
    z_sz = opts.z_sz
    re_sz = opts.re_sz
    scores_low = opts.scores_low
    ...
    for i in range(1, num_frames):
        ...
        if scaled_search_area[0]==scaled_search_area[1]:
            scaled_search_area[1] += 1
        if scaled_search_area[2] <= scaled_search_area[1]:
            scaled_search_area[2]+=2
        scaled_target_w = target_w * scale_factors
        scaled_target_h = target_h * scale_factors
        # try:
        image_, scores_, scores_low_ = sess.run(
            [image, scores, scores_low],
            feed_dict={
                siam.re_h:re_sz[0],
                siam.re_w:re_sz[1],
                siam.pos_x_ph: pos_x,
        ...
        #     print "frame_name_list[i]: ", frame_name_list[i]
        #     print "re_sz: ", re_sz
    ...
    # plt.close('all')
    scores_low_ = np.squeeze(scores_low_)
    return bboxes, speed, scores_, image_, templates_z_, z_sz, scores_low_[1,:,:]
