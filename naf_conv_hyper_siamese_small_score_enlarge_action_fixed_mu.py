...
model_name = 'fix_mu_sigma{}_batch_size{}_enlarge_action_3conv_3fc_punish_reward1'.format(sigma,batch_size)
...
tensorBoard = TensorBoardNaf(log_dir='./logs_tb',histogram_freq=5)
nb_steps = 200000
file_logger = FileLogger(filepath=os.path.join(save_path,'{}_log_{}'.format(model_name, ENV_NAME+str(nb_steps))))
interval_checkpoint = ModelIntervalCheckpoint(filepath=os.path.join(save_path,"{}".format(model_name)+"_weights.{step:06d}.h5f"), interval=10000, verbose=True)
history = agent.fit(env, nb_steps=nb_steps, visualize=False, verbose=1, nb_max_episode_steps=200, callbacks=[file_logger,interval_checkpoint,tensorBoard])
...
agent.mu_model.save(os.path.join(save_path,"{}_mu_model.h5".format(model_name)))
